...
bot.on('photo', ctx =>{
    ...
    photo.forEach((ph, i) =>{
        ctx.reply(`A photo ${i} tem resolução de ${ph.width}x${ph.height}`)
    })
})
...
bot.on('sticker', ctx => {
    ...
    ctx.reply(`Estou vendo que você enviou um ${sticker.emoji} do conjunto ${sticker.set_name}`)
